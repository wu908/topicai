"""Delete the visual-pass temp account from .omni-visual-state.json."""
...
state = json.loads(Path(r"G:\codex_project\topicAI\mvp\scripts\.omni-visual-state.json").read_text())
...
print("account_delete", code, (deleted or {}).get("data", {}).get("status"))
Path(r"G:\codex_project\topicAI\mvp\scripts\.omni-visual-state.json").unlink(missing_ok=True)
print("STATE_CLEANED")
