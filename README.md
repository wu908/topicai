...
- 发布检查绑定锁定版本，展示风险位置、原因、严重级别、规则来源和更新时间；检查过期或发现未确认时不能记录发布。
- 正文、正文文件和配图方案可分别复制或下载；配图方案由浏览器原生 Canvas 导出为 PNG，单项失败可单独重试。
- 表现截图作为敏感图片素材上传；视觉模型只预填指标，必须逐项确认后才能保存。修改 AI 提议的指标会记录为 `edited`，视觉能力不可用时保留手工录入。
- 素材支持文本、链接、图片和文档，带隐私级别、项目引用、复用记录及锁定版本删除影响保护；内容工作台可通过“项目素材”抽屉直接关联已有素材。
- “我的”提供周目标、内容策略、小红书账号引用、AI 能力状态、HumanGate 保护的数据导出与账户删除；删除先撤销凭据，将文件隔离到临时区，数据库失败时恢复文件，提交后才清除文件。
...
当前 lockfile 解析到 Playwright `1.62.0`；升级 Playwright 后需重新执行浏览器安装命令。依赖安全门禁为 `pnpm audit --audit-level high`，不使用会跨主版本升级的强制修复。
