...
class IntentOrchestratorService:
    ...
    def _action_spec(self, action_type: str, project: dict[str, Any] | None) -> dict[str, Any]:
        ...
        specs = {
            ...
            "lock_intent": ("锁定发布意图与发布前判断", "锁定后，发布意图与发布前判断将成为不可修改的发布历史。", ["content:current_version", "project:intent"], ["complete_publish_judgment"], 3, None, {"action_type": "lock_intent", "path": f"/content/{project_id}"}),
            ...
            "add_performance": ("回填这篇内容的真实表现", f"{config['label']}模式要看的是对应的观察信号，不用统一的爆款分衡量。", ["publication:record"], config["signals"], 4, None, {"action_type": "add_snapshot", "path": f"/content/{project_id}"}),
